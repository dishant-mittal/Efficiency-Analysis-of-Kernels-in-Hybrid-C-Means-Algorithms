...
for i in C1:
    x.append(dt[i,27].astype(np.float))
    y.append(dt[i,44].astype(np.float))
...
for i in C2:
    X1.append(dt[i,27].astype(np.float))
    Y1.append(dt[i,44].astype(np.float))
...
for i in C3:
    X2.append(dt[i,27].astype(np.float))
    Y2.append(dt[i,44].astype(np.float))
...
plt.grid(True)
plt.xlabel('Attribute 28',size='17')
plt.ylabel('Attribute 45',size='17')
plt.legend(loc='upper right')
plt.title('RIFCM with Gaussian Kernel',size='18')
