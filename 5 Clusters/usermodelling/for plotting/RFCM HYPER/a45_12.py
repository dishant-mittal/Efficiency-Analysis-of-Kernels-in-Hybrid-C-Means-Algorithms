...
for i in C1:
    x.append(dt[i,45].astype(np.float))
    y.append(dt[i,12].astype(np.float))
...
for i in C2:
    X1.append(dt[i,45].astype(np.float))
    Y1.append(dt[i,12].astype(np.float))

ax1.scatter(X1,Y1,color='green',s=20,edgecolor='none',alpha=0.6,label='Cluster 2')
ax1.set_aspect(1./ax1.get_data_ratio()) # make axes square

C3=d[0:371,2].astype(np.int)
...
for i in C3:
    X2.append(dt[i,45].astype(np.float))
    Y2.append(dt[i,12].astype(np.float))
    
ax1.scatter(X2,Y2,color='black',s=7,edgecolor='none',alpha=0.6,label='Cluster 3')
ax1.set_aspect(1./ax1.get_data_ratio()) # make axes square

C4=d[0:31,3].astype(np.int)
...
for i in C4:
    X3.append(dt[i,45].astype(np.float))
    Y3.append(dt[i,12].astype(np.float))
    
ax1.scatter(X3,Y3,color='red',s=15,edgecolor='none',alpha=0.8,label='Cluster 4')
ax1.set_aspect(1./ax1.get_data_ratio())

Centroid1_x=[51.266503]
Centroid1_y=[35.480374]

ax1.scatter(Centroid1_x,Centroid1_y,color='blue',marker='^',s=70,edgecolor='none',alpha=1)
ax1.set_aspect(1./ax1.get_data_ratio())

Centroid2_x=[22.103579]
Centroid2_y=[27.499865]

ax1.scatter(Centroid2_x,Centroid2_y,color='green',marker='^',s=70,edgecolor='none',alpha=1)
ax1.set_aspect(1./ax1.get_data_ratio())

Centroid3_x=[38.365608]
Centroid3_y=[30.613177]

ax1.scatter(Centroid3_x,Centroid3_y,color='black',marker='^',s=70,edgecolor='none',alpha=1)
ax1.set_aspect(1./ax1.get_data_ratio())

Centroid4_x=[40.211402]
Centroid4_y=[30.837635]
...
plt.grid(True)
plt.xlabel('Attribute 46',size='17')
plt.ylabel('Attribute 13',size='17')
plt.legend(loc='upper right')
plt.title('RFCM with Hypertangent Kernel',size='18')
