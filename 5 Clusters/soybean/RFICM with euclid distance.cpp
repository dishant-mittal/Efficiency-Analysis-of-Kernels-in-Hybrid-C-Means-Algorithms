...
int main(void)
{
 ...
 //FILE READING COMPLETE
    ...
    double c_means[35][5],distance[376][5],c_distance[5][5],U[376][5],diff[376];
    int temp;
  ...
  //initialized distance matrix with 0 
   ...
   for(int y=0;y<5;y++)
   {
          ...
          /* for(int j=0;j<5;j++)
          {
                  for(int k=0;k<35;k++)
                  {       
                          distance[y+5][j]+=pow(((double)array[y+5][k]-c_means[k][j]),2);
                  }
                  distance[y+5][j]=sqrt(distance[y+5][j]);
          }
           for(int j=0;j<5;j++)
                  {
                      summation=0.0;
                      for(int k=0;k<5;k++)
                      {
                           summation+=pow((distance[y+5][j])/(distance[y+5][k]),2);
                      }
                      U[y+5][j]=1/summation;
                  
                  pi=0.0;
                  pi=1.0-U[y+5][j]-((1.0-U[y+5][j])/(1.0+3.0*U[y+5][j]));
                  U[y+5][j]=U[y+5][j]+pi;
                  }*/
   }
